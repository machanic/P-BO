...
from config import IN_CHANNELS, CLASS_NUM, PROJECT_PATH, MODELS_TEST_STANDARD
from dataset.dataset_loader_maker import DataLoaderMaker
from PBO_attack.bo import BayesOpt
    ...
    def attack_all_images(self, args, arch_name, target_model, result_dump_path):
        ...
        for batch_idx, (images, true_labels) in enumerate(self.dataset_loader):
            ...
            if not args.targeted:
                ...
                if args.method == 'bo':
                    adv_images, query = run_bo(obj, bounds, args.max_queries, device=device, prior_obj=prior_obj, scale=args.bo_scale)
            else:
            ...
            if adv_images == None:
                adv_images = data     # 攻击失败
            query = torch.tensor(query).float().cuda()
            with torch.no_grad():
                adv_logit = obj.get_logits(adv_images)
            ...
            log.info("{}-th batch (size={}), current batch success rate:{:.3f}".format(batch_idx, adv_images.size(0),
                                                                                       success.mean().item()))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--json-config', type=str, default='./configures/PBO_attack.json',
    ...
    parser.add_argument('--surrogate-arch', default=None, type=str, required=True, help='network architecture')
    ...
    if args.all_archs:
        ...
        if args.dataset == "CIFAR-10" or args.dataset == "CIFAR-100":
            for arch in MODELS_TEST_STANDARD[args.dataset]:
                test_model_path = "{}/train_pytorch_model/real_image_model/{}-pretrained/{}/checkpoint.pth.tar".format(PROJECT_PATH,
                                                                                        args.dataset,  arch)
